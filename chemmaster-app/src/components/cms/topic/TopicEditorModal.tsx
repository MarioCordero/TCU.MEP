...
import { 
  ...
  BasicTextStyleButton, BlockTypeSelect, ColorStyleButton,
  CreateLinkButton, FileCaptionButton, FileReplaceButton,
  NestBlockButton, UnnestBlockButton, TextAlignButton,
  useEditorSelectionChange, useBlockNoteEditor
} from "@blocknote/react"
import { 
  BlockNoteSchema, defaultBlockSpecs, defaultStyleSpecs, 
  createStyleSpec,
} from "@blocknote/core"
...
function TipTapMarkButton({ mark, label }: { mark: string; label: string }) {
  ...
  return (
    <button
      ...
      className={["bn-button", isActive ? "bn-button-selected" : ""].join(" ")}
      style={{
        cursor: "pointer",
        paddingInline: "8px",
        backgroundColor: isActive ? undefined : isHovered ? "#e9ecef" : "transparent",
        borderRadius: "4px",
    ...
    >
      {mark === "superscript" ? (
        <span>
          A<sup style={{ fontSize: "0.65em", lineHeight: 0 }}>2</sup>
        </span>
      ) : (
        <span>
          A<sub style={{ fontSize: "0.65em", lineHeight: 0 }}>2</sub>
        </span>
      )}
    </button>
  )
...
export default function TopicEditorModal({
  ...
  const schema = useMemo(() => {
    const mathBlockSpec = typeof MathBlock === "function" ? (MathBlock as any)() : MathBlock
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        ...
        setErrorAlert({
          show: true,
          msg: "No se pudo cargar el contenido del tópico. Intenta recargar la página.",
        })
      }
    }
  ...
  const handleSave = async () => {
    if (!editedTopic || !editedTopic.id) {
      setErrorAlert({
        show: true,
        msg: "ID del tópico no encontrado",
      })
      return
    }
    ...
    try {
      ...
      try {
        ...
        console.warn("Could not parse old content for file cleanup")
      }
      ...
      if (filesToDelete.length > 0) {
        await Promise.all(
          filesToDelete.map((url) => {
            ...
            if (filename) return API.File.Delete(filename)
      ...
      setErrorAlert({
        show: true,
        msg: "Error crítico al guardar: " + errorMsg,
      })
    } finally {
      setIsSaving(false)
    }
  ...
  return (
    <MathBlockProvider>
      ...
      {/* Modal Overlay - Hidden when success modal is shown */}
      {!showSuccess && (
