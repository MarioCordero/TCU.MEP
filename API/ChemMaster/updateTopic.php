<?php
    ...
    try {
        ...
        if (!$stmt) throw new Exception("Error preparando la consulta: " . $conn->error);

        // Types: s(title), s(desc), s(content), i(order), i(id)
        $stmt->bind_param("sssii", $title, $description, $content, $order_in_module, $id);
        if (!$stmt->execute()) throw new Exception("Error ejecutando la actualización: " . $stmt->error);
        ...
        if (isset($environment) && $environment === 'development') $errorMsg .= " Detalle: " . $e->getMessage();
