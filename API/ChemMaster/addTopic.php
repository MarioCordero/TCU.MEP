<?php
    ...
    try {
        ...
        $sql = "INSERT INTO topics (module_slug, title, description, content, order_in_module, created_at) 
                VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)";
        
        $stmt = $conn->prepare($sql);
        if (!$stmt) throw new Exception("Error preparando INSERT: " . $conn->error);

        // Types: s (slug), s (title), s (desc), s (content), i (order)
        $stmt->bind_param("ssssi", $module_slug, $title, $description, $content, $order_in_module);
